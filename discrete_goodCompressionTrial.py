...
# Hard-coded UI scaling via UI_SCALE_DEFAULT.
...
# Optional deps for compressed image decode AND raw downsampling
try:
    import numpy as np
    import cv2
    try:
...
UI_SCALE_DEFAULT = 4   # <--- change this to make the whole UI bigger/smaller
...
class XboxTeleopApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # NOTE: we now also use reduced_decode for RAW path downsampling
        ...
        # Loops
        rospy.Timer(rospy.Duration(0.016), self.publish_twist)   # ~60 Hz
        self.after(33, self.video_tick)                          # ~30 FPS
        self.after(150, self._sync_video_width)                  # fit video width
    ...
    def _build_gui(self):
        ...
        self._buttons = []
        for g in range(1, 6):
            normal, pressed = self.GEAR_COLORS[g]
            b = tk.Button(
                self.btn_frame, text=str(g), width=4, bg=normal,
                fg="white" if g >= 4 else "black",
                font=self.font_btn, padx=self.px(8), pady=self.px(4),
                command=lambda gg=g: self._set_gear(gg)
            )
            b.pack(side="left", padx=self.px(5))
            self._buttons.append(b)

        # Row 2: Video display
        self.video_label = tk.Label(mid, bg="#000000")
        self.video_label.pack(pady=self.px(10), padx=self.px(10))
    ...
    def image_callback_raw(self, msg):
        """RAW path: use CvBridge to get BGR, then optionally downsample with OpenCV."""
        try:
            bgr = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')

            # ---- NEW: optional client-side "compression" via downsampling ----
            # Uses same params as compressed path: fast_decode + reduced_decode
            if self.fast_decode and self.reduced_decode in (2, 4) and cv2 is not None:
                h, w = bgr.shape[:2]
                new_w, new_h = w // self.reduced_decode, h // self.reduced_decode
